...
4. **Price → arrivals** (Pillars 1+4, the boundary reopen): segment heat × posture drives FloorSim volume. ✓ armed S7 (#279) — `pricingTrafficWeight` raised 0→1 and `MarketEconomy.demandMultiplierFor` injected as the per-vehicle response of `computePricingTrafficMultiplier`; the shared `demandMultiplier` (S4) now drives both the screen prediction and realized arrivals. Magnitudes deferred to S14.
...
**Ship discipline (S5, #277 → armed S7, #279):** S5 wired the seam at **identity** — `computePricingTrafficMultiplier` returned 1 while `demandModel.pricingTrafficWeight = 0`. **S7 arms it:** `pricingTrafficWeight` is now `1` and the composition root injects the per-vehicle response from MarketEconomy's shared `demandMultiplier` (#276 / Pillar 3 — one model, two consumers) via the new `marketEconomy.demandMultiplierFor(vehicle, askingPrice)`, so the screen's predicted days-to-sell and the floor's actual arrivals read the same curve. Because intake stamps each unit's ask at the market suggestion (#273), a default-priced lot resolves to ≈1 (no swing until the player re-prices), and the neutral-heat calibration path (#94) stays byte-identical. Elasticity *magnitudes* (steepness, heat-volume coupling, the weight itself) are deferred to S14. The `buildPricingInfluence` segment-*skew* socket (which segment walks in) remains inert — that's a later slice, distinct from this volume seam.
