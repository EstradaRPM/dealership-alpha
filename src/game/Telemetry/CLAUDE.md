...
- `fniAttachRate` — % of closed deals where `backGross > 0`. A later telemetry pass can break this down by product once F&I product detail is published on a dedicated event.
...
## Data
- `data/telemetry.json` — `cashCurveBucketDays` (reserved; currently always 1), `maxBufferedEvents` (hard cap to bound memory in long sessions).
